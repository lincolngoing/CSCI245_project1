...
#include <stdio.h>
#include <stdlib.h>
//main counting comparisons variable
int count = 0;
int
...
{
  //int i tracks the splice in between where ints <= and ints > are stored in the array
  int i = start;
  //int j tpracks the slice in between sorted and unsorted integers
  int j;
  //int t stores the int being swapped
  int t;
  //pivot is the int that the rest of the array is compared to0
  int pivot = array[stop-1];
 
  //This for loop runs through all unsorted items in the array
  for(j = start; j < stop; j++){
    count++;
    // printf("%d\n",array[i]);
    //printf("%d\n\n",i);

    //this if compares the next unsorted item with the pivot
    if(array[j] <= pivot){
      t = array[i];
      array[i] = array[j];
      array[j] = t;
  ...
  //recursion if there are values in the array less than or equal to the pivot
  count++;
  if (start+2 < i){
  quickSortR(array, start, i-1);
  } 
  //recursion if there are values in the array greater than the pivot
  count++;
  if(i <  stop-1){
    quickSortR(array, i, stop);
   }
